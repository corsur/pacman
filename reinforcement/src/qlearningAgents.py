...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        if (state,action) not in self.learned: 
            self.learned[(state,action)] = 0.0
    ...
    def computeValueFromQValues(self, state):
        ...
        if legalActions:
            maxAction = max(legalActions, key=lambda act:self.getQValue(state, act))
    ...
    def computeActionFromQValues(self, state):
        ...
        if self.getLegalActions(state):
            ...
            for action in self.getLegalActions(state):
                if not rightAction:
                    rightAction = action
                elif self.getQValue(state, action) > self.getQValue(state, rightAction):
                    rightAction = action
            return rightAction
        else:
            return None
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        self.learned[(state, action)] = (1-self.alpha)*self.getQValue(state, action)+(self.alpha*(reward+(self.discount*self.computeValueFromQValues(nextState))))
