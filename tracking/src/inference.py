        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            ...
            for newPosition in newPositionDistribution:
                newBeliefs[newPosition] += (newPositionDistribution[newPosition]*self.beliefs[position])
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particleList = []
        for index,position in enumerate(self.legalPositions):
            if index<(numParticles%numPositions):
                self.particleList = self.particleList+([position]*((numParticles/numPositions)+1))
            else:
                self.particleList = self.particleList+([position]*(numParticles/numPositions))
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            newParticles = newParticles+([self.getJailPosition()]*self.numParticles)
            self.particleList = newParticles
        else:
            for particle in self.particleList:
                trueDistance = util.manhattanDistance(particle,pacmanPosition)
                if 0<emissionModel[trueDistance]:
                    beliefs[particle] += emissionModel[trueDistance]
            
            "newParticles = util.nSample(emissionModel,beliefs,self.numParticles)"
    ...
    def elapseTime(self, gameState):
        ...
        newParticleList = []
        for particle in self.particleList:
            newParticleList.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, particle))))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for element in cartesianProduct:
            meh += [element]
        numParticles = self.numParticles
        numPositions = len(meh)
        random.shuffle(meh)
        self.particleList = []
        for index,position in enumerate(meh):
            if index<(numParticles%numPositions):
                self.particleList = self.particleList+([position]*((numParticles/numPositions)+1))
            else:
                self.particleList = self.particleList+([position]*(numParticles/numPositions))
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i]==None:
                ...
                for particle in self.particleList:
                    ...
                    if 0<beliefs[particle]:
                        beliefs[particle] *= emissionModels[i][trueDistance]
                    else: 
                        beliefs[particle] += emissionModels[i][trueDistance]"""
        if type(max(noisyDistances)) is int:             
            for particle in self.particleList:
                tempBeliefs = []
                for i in range(self.numGhosts):
                    if noisyDistances[i]==None:
                        ...
                        trueDistance = util.manhattanDistance(particle[i],pacmanPosition)
                        tempBeliefs += [emissionModels[i][trueDistance]]
                if tempBeliefs:
                    beliefs[particle] += reduce(lambda x, y: x*y, tempBeliefs)
            ...
            for i in eaten:
                newParticles = []
                for particle in self.particleList:
                    newParticles.append(self.getParticleWithGhostInJail(particle, i))
                self.particleList = newParticles
