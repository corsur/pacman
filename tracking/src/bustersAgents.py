...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDistribution in livingGhostPositionDistributions:
            ...
            distance = self.distancer.getDistance(ghostPosition,pacmanPosition)
            if not closest:
                closest = [ghostPosition,distance]
        ...
        for action in legal:
            ...
            newDistance = self.distancer.getDistance(successorPosition,closest[0])
            if newDistance==0:
                return action
            if not bestAction:
                bestAction = [action,newDistance]
            elif bestAction[1]>newDistance:
                bestAction = [action,newDistance]
        return bestAction[0]